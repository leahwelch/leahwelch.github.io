...
var svg = d3.select("#graph")
  ...
  d3.queue()
    ...
    function sec_3() {
        canvas_clear();
        var lossesN = nodes.filter(function(d) {
            return d.filterId === 2;
        });

        var lossesL = links.filter(function(d) {
            return d.filterId === 2;
        });

        var simulation = d3.forceSimulation(lossesN)
            .force("link", d3.forceLink(lossesL).id(function(d) { return d.id; }).distance(50).strength(1))
            .force("charge", d3.forceManyBody().strength(-200))
            .force("center", d3.forceCenter(width/2, height/2))
            .force("collide", d3.forceCollide().radius(50));
