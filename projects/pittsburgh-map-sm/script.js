...
Promise.all(promises).then(function (geoData) {
  ...
  const filtered = pittCoords.filter(function (d) {
    return d.coordinates[0] > xMin && d.coordinates[0] < xMax && d.coordinates[1] > yMin && d.coordinates[1] < yMax;
  })

  filtered.forEach(function (d) {
    return d.score = Math.random() * 6;
  })
